...
public final class Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            JSONObject jo = new JSONObject(br.lines().reduce("",String::concat));
    ...
    private void saveStudent(HttpServletResponse response, JSONObject jsonStudent) 
            throws IOException, SQLException, ClassNotFoundException {
            try{
                PrintWriter pw = response.getWriter();
                new StudentBO().saveStudent(Student.fromJson(jsonStudent));
                response.setContentType("application/json");
                ...
                if(ex.getErrorCode() == 1062){
                    PrintWriter pw = response.getWriter();
                    pw.write("{\"msg\": \"Error: Student with id:".concat(Student.fromJson(jsonStudent).getId()).concat(" already exists.\"}"));
                    pw.flush();                
                }else{
                    throw ex;
                }                
